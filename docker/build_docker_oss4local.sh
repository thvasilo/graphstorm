...
# ensure USE_PARMETIS is either true or false
if [[ $USE_PARMETIS != "true" && $USE_PARMETIS != "false" ]]; then
    echo "USE_PARMETIS must be either true or false, but got \"$USE_PARMETIS\""
    exit 1
fi
...
# Copy DGL tools from one level above
mkdir -p $GSF_HOME"/docker/code/dgl"
cp -r $GSF_HOME"/../dgl/tools" $GSF_HOME"/docker/code/dgl/tools"
