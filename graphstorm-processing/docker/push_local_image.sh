...
if [ -b "${1-}" ] && [ "$1" == "--help" ] || [ -b "${1-}" ] && [ "$1" == "-h" ]; then
    echo "Usage: docker/push_local_image.sh"
    echo "Optionally provide the image name, tag, region and account number for the ECR repository"
    echo "For example: docker/push_local_image.sh graphstorm-processing 0.1.1 us-west-2 1234567890"
    exit 1
fi

latest_version=`poetry version --short` # needs to be updated anytime there's a new version
