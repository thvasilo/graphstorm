...
def determine_spark_feature_type(feature_type: str) -> Type[DataType]:
    ...
    if feature_type in [
        ...
        return StringType
    if feature_type in ["numerical", "bucket_numerical", "none"]:
        return FloatType
    else:
...
def _parse_edge_labels_schema(edge_labels_objects: Sequence[LabelConfig]) -> Sequence[StructField]:
    ...
    for label_config in edge_labels_objects:
        if len(label_config.cols) > 1:
            raise NotImplementedError("For now we are only supporting single-col labels")
