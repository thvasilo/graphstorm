...
The script reads the generated metadata.json output of
the distributed processing pipeline, and re-partitions the
non-conforming files so that all corresponding part-* features and edges files
of the same edge type have the same number of rows.
...
class ParquetRepartitioner:
    ...
    def read_parquet_from_relative_path(self, relative_path: str) -> pyarrow.Table:
        """
        Read a parquet file from S3, prepending the self.s3_prefix_with_bucket to the object path.
        """
    ...
    def repartition_parquet_files_in_memory(
        ...
        The assumed format of the `data_entry_dict` is:
        {
            "format": {
                "name": "parquet"
            },
        ...
        if table.num_rows != sum(desired_counts):
            raise RuntimeError(
                "Data on S3 had a different total row count "
...
def collect_frequencies_for_data_counts(data_meta: Dict) -> Dict[str, Counter]:
    ...
    Note that the row counts are converted to tuples to
...
def verify_metadata(
    ...
    if edge_structure_meta and edge_data_meta:
        edges_match = ParquetRowCounter.verify_features_and_structure_match(
    ...
    if node_data_meta is not None:
        ...
        if not nodes_match:
            logging.error(
                "Node data row count metadata did not match with expected, check produced output"
            )
...
def parse_args(args):
    """
    Get input location in S3 and filename of metadata file.
    """
...
def main():
    ...
    if filesystem_type == "s3":
        input_prefix = args.input_prefix[:-1] if args.input_prefix[-1] == "/" else args.input_prefix
    ...
    # Re-partition the edge files based on the most popular row counts for each edge type
    if edge_data_exist:
