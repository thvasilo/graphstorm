...
def test_multiple_values():
    args = ['--target-etype', 'query,clicks,asin', 'query,search,asin', '--feat-name', 'ntype0:feat0', 'ntype1:feat1']
    expected = {
...
def test_no_args():
    ...
    assert len(result) == 0
    assert result['non_existing'] == ''  # This demonstrates defaultdict behavior
...
def test_defaultdict_behavior():
    ...
    assert result['non_existing'] == ''  # This would raise KeyError with a regular dict
